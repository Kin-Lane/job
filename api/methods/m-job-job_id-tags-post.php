...
$app->post($route, function ($job_id)  use ($app){
	...
	if(isset($param['tag']))
		...
		$CheckTagPivotQuery = "SELECT * FROM job_tag_pivot where Tag_ID = " . trim($tag_id) . " AND Job_ID = " . trim($job_id);
		...
		if($CheckTagPivotResult && mysql_num_rows($CheckTagPivotResult))
			...
			{
			$query = "INSERT INTO job_tag_pivot(Tag_ID,Job_ID) VALUES(" . $tag_id . "," . $job_id . "); ";
			mysql_query($query) or die('Query failed: ' . mysql_error());
			}
